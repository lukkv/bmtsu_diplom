...
def home():
    ...
    if request.method == 'POST':
        ...
        if error == '':
            ...
            preprocessor3 = load_pickle_obj('preprocessor_mn')
            model3 = load_pickle_obj('model_mn.pkl')
            x3 = preprocessor3.transform(x)
            y3 = model3.predict(x3)
            var1 = y3[0]

    return render_template('model.html', params=params, error=error, inputs=x.to_html(), var1=var1)
